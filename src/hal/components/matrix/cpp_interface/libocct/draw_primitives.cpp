...
//! Euler angles in radians. Use -> value*toRadians Example function input values: ashape,{0,0,0},0,0,90*toRadians
...
//! Transparancy value 0.0 - 1.0
Handle(AIS_Shape) draw_primitives::transparancy(Handle(AIS_Shape) shape, double transparancy){
    shape->SetTransparency(transparancy); // 0.0 - 1.0
    return shape;
}
