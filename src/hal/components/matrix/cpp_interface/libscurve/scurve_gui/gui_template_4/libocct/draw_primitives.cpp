...
Handle(AIS_Shape) draw_primitives::draw_3d_line(gp_Pnt point1, gp_Pnt point2){
    ...
    // Avoid zero linelenghts. Will result in error.
    double lenght=sqrt(pow(point2.X()-point1.X(),2)+pow(point2.Y()-point1.Y(),2)+pow(point2.Z()-point1.Z(),2));
    if(lenght==0){
...
Handle(AIS_Shape) draw_primitives::draw_2d_text(std::string str, double textheight, gp_Pnt point, double rot_deg){ //https://www.youtube.com/watch?v=31SXQLpdNyE

    const char *chartext=str.c_str();

    if(textheight==0 || textheight<0){textheight=0.01;}
