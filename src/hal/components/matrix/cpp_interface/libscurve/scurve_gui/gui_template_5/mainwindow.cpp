...
Result<T> motion(T s, T vo, T ve, T vs, T a, T at_time, T gain){
    ...
    if(vo>=vs && vs>=ve && vs>0 && s>0){
        ...
        //! Dcc period to vs.
        r=scurve_bck(gain,a,vo,vs,0.0);
        t1=r.sc_ct;
        s1=r.sc_cs;

        //! Dcc vs to ve period.
        r=scurve_bck(gain,a,vs,ve,0.0);
        t3=r.sc_ct;
        s3=r.sc_cs;
        ...
        if(t<t1){ //! Period 1.
            if(prompt_new_ve){
                 r=scurve_bck(100.0/*gain*/,a,vo,ve,t);
                 r.tr_ct=ct;
                 r.tr_cs=cs;
                 return r;
            }
    ...
    if(vo<=vs && vs>0 && s>0){
        ...
        if(t<t1){ //! Period 1.
            if(prompt_new_ve){
                r=scurve_fwd(100.0/*gain*/,a,vo,ve,t);
                r.tr_ct=ct;
                r.tr_cs=cs;
                return r;
            }
...
void MainWindow::on_pushButton_run_pressed()
{
    ...
    std::cout<<"traject time:"<<r.tr_ct<<std::endl;
    ui->label_curve_time->setText(QString::number(r.tr_ct,'f',3)+" sec");

    for(double t=0; t<r.tr_ct; t+=0.01){
